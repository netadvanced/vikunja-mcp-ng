  ...
  describe('createAuthRequiredError', () => {
    it('should create standardized auth required error', () => {
      ...
      expect(result.message).toBe(
        'Authentication required. Please connect first:\n' +
    ...
    describe('Error Message Sanitization', () => {
      it('should sanitize error messages containing file paths', () => {
        const error = new Error(
          'Failed to open file /Users/eringreen/Development/vikunja-mcp/src/config/secrets.json: Permission denied',
        );
      ...
      it('should sanitize database errors with schema information', () => {
        const error = new Error(
          "ER_NO_SUCH_TABLE: Table 'vikunja_production.user_tokens' doesn't exist in database mysql://user:password@localhost:3306/vikunja",
        );
      ...
      it('should sanitize authentication errors with mechanism details', () => {
        const error = new Error(
          'JWT validation failed: signature verification error using key from /etc/keys/jwt-public.pem',
        );
      ...
      it('should sanitize stack traces and internal system details', () => {
        ...
        expect(result.message).not.toContain(
          '/Users/eringreen/Development/vikunja-mcp/src/utils/config.js',
        );
      ...
      it('should sanitize API endpoint structures', () => {
        const error = new Error(
          '404 Not Found - GET https://api.vikunja.example.com/api/v1/projects/123/tasks?status=completed&limit=50 failed',
        );
