...
> **Archived 2026-08-03 — historical record, not current guidance.** This guide was written for
> the upstream package `@democratize-technology/vikunja-mcp` when it went `0.1.x → 0.2.0`; that
> upgrade is four years of history behind this fork, which ships as `vikunja-mcp-ng` (`0.6.2` at
> archive time) and has no `0.1.x` users to migrate. Everything below is preserved verbatim as the
> record of that migration — **do not follow it as configuration advice**: the package name,
> repository URLs, and the "new optional configuration" env vars (`CIRCUIT_BREAKER_*`,
> `FILTER_MAX_*`, `MEMORY_PROTECTION_ENABLED`) it advertises are all absent from current `src/`
> (verified by grep, 2026-08-03) and are silently ignored by this server. For real configuration,
> see [docs/CONFIGURATION.md](../CONFIGURATION.md); for release-to-release upgrade notes, see
...
- **Architecture Details**: See [ARCHITECTURE_SIMPLIFICATION.md](ARCHITECTURE_SIMPLIFICATION.md) *(also archived here)*
