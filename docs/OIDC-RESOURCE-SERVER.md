...
- **`status`** — reports, for the calling `sub`: `linked: true/false`, masked token prefix, `provisionedAt`, `lastUsedAt`, and the record's `keyVersion`. Never reveals another user's status, and never a vault-wide count (that is the operator's boot-time summary, below). When the caller's own record is usable but outdated — the pre-AAD `keyVersion: 1` shape (#262), or a JWT stored before #322's guard existed — it also carries `needsMigration: true` plus a `migrationNotice` telling that user to re-provision. Vault-wide, the operator gets a one-line count of unmigrated records at startup (`VaultFileStore.getMigrationSummary`, logged by `setupOidcHttpAuth`), instead of relying on a per-identity warning that only fires when that identity happens to make a request (#322 finding 2).
