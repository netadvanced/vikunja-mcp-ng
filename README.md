...
- **Global read-only mode.** Set `readOnly: true` in `vikunja-mcp.config.json` (or `VIKUNJA_MCP_READ_ONLY=true`) to keep every tool registered and visible but reject every write/destructive subcommand at dispatch — reads keep working normally. One shared guard, built from a per-tool `subcommand -> read | write | destructive` classification table, drives both this gate and the MCP `readOnlyHint`/`destructiveHint`/`idempotentHint` tool annotations, so they can never drift apart. See [Global Read-Only Safety Mode](docs/CONFIGURATION.md#global-read-only-safety-mode).
