  ...
  private async handleUserAssignment(
    ...
    if (userIds.length > 0 && createdTask.id) {
      try {
        ...
        // unassign everyone (democratize-technology/vikunja-mcp#15).
        // Sequential on purpose (post-#89 pattern sweep): concurrent
        // per-user writes to the same task risk "database is locked" 500s
